...
std::shared_ptr<ConstantNode> Parser::parseNumber() {
  auto current_loc = current;
  if (match(TokenType::NUMBER)) {
    std::string num = static_cast<NumberToken*>(previous())->Val;
    auto result = std::make_shared<ConstantNode>(num);
    return result;
  } else {
    current = current_loc;
    return nullptr;
...
//  expect("{");
//
//  auto StmtLst = parseStmtList();
//
//  expect("}");
//
//  return std::make_shared<ProcedureNode>(procName, std::move(StmtLst));
//}
//
//StmtLst Parser::parseStmtList() {
//  StmtLst stmts;
//  while (true) {
...
//std::shared_ptr<AssignmentNode> Parser::parseAssign() {
//  auto Var = parseVariable();
//
//  if (!Var) {
...
//  expect(";");
//
//  return std::make_shared<AssignmentNode>(std::move(Var), std::move(Expr));
//}
//
...
//  auto StmtLst = parseStmtList();
//  expect("}");
//  return std::make_shared<WhileNode>(std::move(condExpr), std::move(StmtLst));
//}
//
...
//  }
//
//  return std::make_shared<ProgramNode>(std::move(proc_list));
//}
